...
public interface PublisherRepository extends CrudRepository<Publisher, String> {
    ...
    @Transactional
    @Modifying
    @Query("INSERT INTO testdb.publisher (name, address, phone) VALUES (:name, :address, :phone)")
    void insert(@Param("name") String name, @Param("address") String address, @Param("phone") String phone);


    @Transactional
    @Modifying
    @Query("UPDATE testdb.publisher SET name = :newName, address = :address, phone = :phone WHERE name = :oldName")
